'use client';

import { useSectionInView } from '../../../lib/hooks';
import portfolioImg from '../../../../../public/images/myphoto.jpg';
import { smoothScrollTo } from '../../../lib/utils';
import SectionDivider from '../../shared/section-divider';
import SectionHeading from '../../shared/section-heading';
import { motion, useScroll, useTransform } from 'framer-motion';
import Image from 'next/image';
import { useRef } from 'react';
import Link from 'next/link';

export default function About() {
  const { ref } = useSectionInView('about', 0.4);
  const divRef = useRef<HTMLDivElement>(null);

  const { scrollYProgress } = useScroll({
    target: divRef,
    offset: ['0 1', '1.33 1'],
  });
  const scaleProgess = useTransform(scrollYProgress, [0, 1], [0.8, 1]);
  const opacityProgess = useTransform(scrollYProgress, [0, 1], [0.6, 1]);

  return (
    <motion.section
      className="z-50 flex h-[1000px] w-full flex-col items-center justify-start leading-8 dark:bg-darkBg dark:text-white md:scroll-mt-4 lg:h-[1100px] lg:scroll-mt-24"
      ref={ref}
      initial={{ opacity: 0, y: 100 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.175, ease: 'easeInOut' }}
      id="about"
    >
      <div className="flex w-full flex-col items-center pt-8">
        <SectionHeading>About Me</SectionHeading>
        <motion.div
          className="w-full overflow-hidden px-4 py-12 sm:w-[60%] sm:text-center lg:h-[700px] lg:w-[1040px] xl:w-[1180px]"
          ref={divRef}
          style={{
            scale: scaleProgess,
            opacity: opacityProgess,
            willChange: 'transform, opacity',
            transform: 'translateZ(0)',
          }}
        >
          <div className="antialiased group relative w-full">
            <div className="text-md relative z-40 flex flex-col gap-3 font-semibold tracking-wide text-primary lg:absolute lg:right-0 lg:top-[27%] lg:block lg:max-w-[580px] lg:text-start lg:text-lg xl:top-1/3 xl:h-[442px] xl:max-w-[650px]">
              <div className="flex h-full flex-col justify-center gap-6">
                <span>
                  I’m a Frontend Developer specializing in TypeScript, React, and Next.js, building scalable and user-focused web applications.  I create intuitive, high-performance digital experiences that connect products with people.
                </span>
                <span>
                  I’m currently open to web development opportunities where I can contribute, grow, and build impactful solutions.
                </span>
                <p className="flex flex-col items-start sm:items-center lg:items-start">
                  <span>If you’re looking for someone passionate about crafting modern web experiences,</span>
                  <Link
                    href={'contact'}
                    onClick={(e) => {
                      smoothScrollTo({ e, id: 'contact' });
                    }}
                    className="w-52 lg:w-50"
                  >
                    <span className="bg-[#06314f] p-1 text-center text-white text-2xl rounded-sm font-semibold uppercase dark:bg-[#06314f] lg:normal-case">
                      let’s connect!
                    </span>
                  </Link>
                </p>
              </div>
            </div>
           <div className="absolute z-30 hidden lg:left-0 lg:top-1/4 lg:block">
                <motion.div
                  animate={{
                    y: [0, -10, 0], // floating up and down
                  }}
                  transition={{
                    duration: 6,
                    repeat: Infinity,
                    ease: "easeInOut",
                  }}
                  className="relative 
                    h-72 w-72 
                    lg:h-[380px] lg:w-[380px] 
                    xl:h-[470px] xl:w-[470px]"
                >
                  <div
                    className="
                      relative h-full w-full overflow-hidden
                      rounded-[60%_40%_30%_70%/60%_30%_70%_40%]
                      shadow-2xl
                    "
                  >
                    {/* Gradient Overlay */}
                    <div className="
                      absolute inset-0 z-20 
                      bg-gradient-to-b 
                      opacity-70
                      from-[#9ed3ff] 
                      via-[#0762ad] 
                      to-[#9ed3ff] 
                    " />

                    {/* Image */}
                    <Image
                      src={portfolioImg}
                      alt="portfolio image"
                      placeholder='blur'
                      fill
                      className="object-cover opacity-100"
                    />
                  </div>
                </motion.div>
              </div>
          </div>
        </motion.div>
      </div>
      <SectionDivider />
    </motion.section>
  );
}
